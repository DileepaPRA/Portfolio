...
interface BlobDef {
  baseLeft: string; baseTop: string;
  sizeFrac: number; blur: number; opacity: number;
  rx1: number; fx1: number; mx1: number; fmx1: number; px1: number;
  rx2: number; fx2: number; mx2: number; fmx2: number;
  rx3: number; fx3: number; px3: number;
  ry1: number; fy1: number; my1: number; fmy1: number; py1: number;
  ry2: number; fy2: number; my2: number; fmy2: number;
  ry3: number; fy3: number; py3: number;
}
...
const S7 = 2.6457513110;

function blob(
  baseLeft: string, baseTop: string, sizeFrac: number, blur: number, opacity: number,
  base: number, m1: number, m2: number,
  px1: number, py1: number, px3: number, py3: number,
): BlobDef {
  return {
    baseLeft, baseTop, sizeFrac, blur, opacity,
    rx1: 0.13, fx1: base,       mx1: m1, fmx1: base / (φ * φ), px1,
    rx2: 0.06, fx2: base * φ,   mx2: m2, fmx2: base / S3,
    rx3: 0.04, fx3: base * S2,  px3,
    ry1: 0.12, fy1: base * S3 / S2, my1: m1 * 0.9, fmy1: base / (φ + 1), py1,
    ry2: 0.05, fy2: base * S5,      my2: m2 * 1.1, fmy2: base / S7,
    ry3: 0.03, fy3: base / φ,       py3,
  };
}

const VARIANTS: Record<string, BlobDef[]> = {
  a: [
    blob("8%",  "4%",  0.52, 90, 0.21, 0.27, 1.9, 1.3, 0.0, 1.10, 0.4, 2.3),
    blob("50%", "30%", 0.38, 100, 0.14, 0.34, 2.2, 1.6, 2.0, 0.50, 1.7, 0.8),
    blob("28%", "60%", 0.24, 85,  0.11, 0.43, 1.6, 1.1, 1.3, 2.40, 3.1, 1.5),
  ],
  b: [
    blob("43%", "-4%", 0.50, 95,  0.20, 0.29, 2.0, 1.4, 0.8, 0.30, 2.2, 3.7),
    blob("-4%", "38%", 0.36, 105, 0.14, 0.38, 1.8, 1.5, 3.1, 1.70, 0.5, 2.9),
    blob("60%", "56%", 0.22, 85,  0.11, 0.50, 2.4, 1.2, 0.6, 2.00, 1.4, 0.3),
  ],
  c: [
    blob("53%", "7%",  0.48, 90,  0.21, 0.31, 2.1, 1.7, 1.5, 0.70, 3.4, 1.1),
    blob("4%",  "-7%", 0.34, 100, 0.14, 0.40, 1.7, 1.3, 2.3, 1.20, 0.9, 3.2),
    blob("32%", "58%", 0.24, 85,  0.11, 0.48, 2.3, 1.0, 0.4, 3.00, 2.0, 0.6),
  ],
  d: [
    blob("11%", "18%", 0.53, 100, 0.19, 0.25, 2.0, 1.5, 2.5, 0.90, 1.2, 3.5),
    blob("53%", "-4%", 0.36, 105, 0.14, 0.36, 1.9, 1.4, 1.0, 2.20, 3.8, 0.7),
    blob("25%", "63%", 0.23, 85,  0.11, 0.46, 2.5, 1.2, 0.2, 1.80, 0.6, 2.4),
  ],
};
...
export default function SectionAmbient({ color, variant = "a", icons = [] }: Props) {
  ...
  useEffect(() => { iconsRef.current = icons; });
